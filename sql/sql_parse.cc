...
int idb_vtable_process(THD* thd, ulonglong old_optimizer_switch)
{
	...
	else
	{
		...
		lex_start(thd);
		thd->reset_for_next_command();
		if (query_cache_send_result_to_client(thd, thd->query(), thd->query().length) <= 0)
		{
			sp_cache_enforce_limit(thd->sp_proc_cache, stored_program_cache_size);
			sp_cache_enforce_limit(thd->sp_func_cache, stored_program_cache_size);
			Parser_state parser_state;
					...
					while (INFINIDB_execute)
					{
						...
						if (args->elements > 0)
						{
							...
							for (uint i= 0 ; i < args->elements ; i++)
							{
								...
								std::string arg_val(arg_item->val_str(str_buffer)->c_ptr(), arg_item->val_str(str_buffer)->length());
            ...
            /*    else if (thd->lex->sql_command == SQLCOM_EXECUTE || thd->get_command() == COM_STMT_EXECUTE)
				{
					...
					if ( stmt )
					{
                        ...
                        Copy_query_with_rewrite acc(thd, stmt->query(), stmt->query().length, &tmp_query);
				...
				else // normal INFINIDB_execute
				{
					...
					if (p1 != std::string::npos)
					{
						...
						{
							strxnmov(path, FN_REFLEN-1, mysql_real_data_home, thd->db().str ? thd->db().str : "",
							NullS);
						}
